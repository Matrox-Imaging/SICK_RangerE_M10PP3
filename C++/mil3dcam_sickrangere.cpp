...
MIL_INT MFTYPE M3dcamRangerToMIL(MIL_ID MilRangerImage,
   ...
   /* Conditions relative to SICK Ranger E camera. */
   ...
   /* Choose correct conversion routine according to Ranger E range mode. */
   if (ConvParamsPtr->LaserMeasurementMode == RANGER_Hi3D_COG_MODE)
      {
      MIL_INT Ret;
      if (ConvParamsPtr->FlipY == DO_FLIP_Y_AXIS)
...
MIL_INT DoHi3DToMIL(MIL_BUFFER_INFO RangerBuf,
   ...
   /* For all pixels in the Ranger E image... */
...
bool GetBufInfoAndValidate(MIL_ID MilBuf, MIL_BUFFER_INFO* ppBuf)
   ...
   if (MilBuf != M_NULL)
      ...
      MobjInquire(MilBuf, M_OBJECT_TYPE, &ObjectType);
