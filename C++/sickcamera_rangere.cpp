...
MIL_INT FillParams(SSickCameraSystem* pCameraSystem,
                   ...
                   const std::string&)
   {
   pConvParams->LaserMeasurementMode         = RANGER_Hi3D_MODE;
...
MIL_INT CreateCamera(EthernetCamera** ppCam)
   ...
   *ppCam = NULL;
   ...
   /* Call camera factory in the icon api. Ask for a Ranger E camera. */
   Camera* pGeneric = createCamera(EthernetCamera::cameraType, "MyCamera");

   /* Check result from factory and cast it to a Ranger E camera. */
