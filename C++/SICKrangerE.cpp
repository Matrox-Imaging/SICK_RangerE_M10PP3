...
void PrintHeader()
   ...
   MosPrintf(MIL_TEXT("[EXAMPLE NAME]\n")
             ...
             MIL_TEXT("Ranger E camera and scans an object to generate its depth map.\n\n")
...
/*****************************************************************************
 Function that calibrates a 3d reconstruction setup with a SICK Ranger E camera
...
void RangerEExample(MIL_ID MilSystem, MIL_ID MilDisplay, MIL_ID MilDepthMapDisplay)
   ...
   MosPrintf(MIL_TEXT("\n3D RECONSTRUCTION USING SICK RANGER E:\n"));
   MosPrintf(MIL_TEXT("------------------------------------\n\n"));
   MosPrintf(MIL_TEXT("This program calibrates a 3d reconstruction setup with a SICK\n"));
   MosPrintf(MIL_TEXT("Ranger E camera and scans an object to generate its depth map and\n"));
   ...
   /* Free result buffer for calibration. */
   M3dmapFree(MilCalibScan);
   MilCalibScan = M_NULL;

   /* Allocate result buffer for scanning. */
...
void CreateColorMap(MIL_ID MilSystem,
   ...
   /* Get the calibration information from the depth map */
...
/*****************************************************************************
 Uses MimDraw() to show the image of the laser line the SICK Ranger E has extracted.
...
void CalculateColorBandsAverage(MIL_ID MilColorImage, MIL_DOUBLE BandAverage[3])
   {
   MIL_ID MilSystem      = MbufInquire(MilColorImage, M_OWNER_SYSTEM, M_NULL);
   ...
   for(MIL_INT BandIdx = 0; BandIdx < 3; BandIdx++)
      {
      MIL_ID MilColorBand = MbufChildColor(MilColorImage, BAND_COLOR[BandIdx], M_NULL);
      MimStatCalculate(MilStatContext, MilColorBand, MilStatResult, M_DEFAULT);
      MimGetResult(MilStatResult, M_STAT_MEAN + M_TYPE_DOUBLE, &BandAverage[BandIdx]);
      MbufFree(MilColorBand);
      }
